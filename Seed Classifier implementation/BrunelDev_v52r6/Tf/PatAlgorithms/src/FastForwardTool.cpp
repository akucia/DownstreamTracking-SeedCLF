                                  ...
                                  std::vector<LHCb::Track*>& output ) const
{
  ...
  if ( seed.checkFlag( LHCb::Track::Flags::Invalid  ) ) return StatusCode::SUCCESS;
  if ( seed.checkFlag( LHCb::Track::Flags::Backward ) ) return StatusCode::SUCCESS;
  ...
  for ( auto itL = goodCandidates.begin(); goodCandidates.end() != itL; ++itL ) {
    ...
    fwTra->addInfo(LHCb::Track::AdditionalInfo::PatQuality, (*itL).quality());
    // added for NNTools
    fwTra->addInfo(LHCb::Track::AdditionalInfo::Cand1stQPat, (*itL).cand1stquality());
    fwTra->addInfo(LHCb::Track::AdditionalInfo::Cand2ndQPat, (*itL).cand2ndquality());
...
void FastForwardTool::fillXList ( PatFwdTrackCandidate& track, double xMin, double xMax ) const {
  ...
  for (unsigned int sta = 0; sta < m_nSta; sta ++){
    ...
    std::sort( inStation.begin(), inStation.end(), Tf::increasingByProjection<>() );
...
double FastForwardTool::getTTOffset ( PatFwdTrackCandidate& track, bool isDebug )  const {
  ...
  std::sort( ttHits.begin(), ttHits.end(), Tf::increasingByProjection<>() );
...
bool FastForwardTool::fillStereoList ( PatFwdTrackCandidate& track, double tol )  const {
  ...
  std::sort( temp.begin(), temp.end(), Tf::increasingByProjection<>() );
  ...
  //== Sort by Z
  std::sort( track.coordBegin(), track.coordEnd(), Tf::increasingByZ<>() );

...
void PatSeedFit::handle(const Incident& incident)
{
  ...
  // end of the event
  if (IncidentType::EndEvent == incident.type()) m_stLiteContainer = nullptr;
}

namespace {
struct sortadapter {
	bool operator()(const PatFwdHit& h1, const PatFwdHit& h2) const
	{ return Tf::increasingByZ<>()(&h1, &h2); }
};
}
				...
				std::vector<LHCb::State> *states) const
{
  ...
  for(LHCb::LHCbID hit: lhcbIDs) {
    switch (hit.detectorType()) {
        case LHCb::LHCbID::IT : {
            ...
            if(!stSector) return Error( "No sector found for IT hit!" );
            auto iclus = m_stLiteContainer->find< LHCb::STLiteCluster::findPolicy >( stChan ) ;
            if( iclus == m_stLiteContainer->end() ) return Error("Cannot find lite cluster!");
  ...
  std::sort(hits.begin(), hits.end(), sortadapter());
  ...
  for( PatFwdHit& ihit: hits ) {
      ...
      if (ihit.isOT()) ++planectrOT[ihit.planeCode()];
      else             ++planectrIT[ihit.planeCode()];

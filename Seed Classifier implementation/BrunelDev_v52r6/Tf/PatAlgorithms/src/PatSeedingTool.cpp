...
unsigned PatSeedingTool::prepareHits()
{
  ...
  for( PatFwdHit* hit: range ) {
    ...
    if ( !m_reusePatSeeding ) {
      if ( hit->hit()->testStatus(Tf::HitBase::EStatus::UsedByPatSeeding) ) {
        hit->setIsUsed(true);
        continue;
      }
    }
                                  ...
                                  std::vector<LHCb::Track*>& outputTracks ) const
{
  ...
  out->setType( LHCb::Track::Types::Ttrack );
  out->setHistory( LHCb::Track::History::PatSeeding );
  out->setPatRecStatus( LHCb::Track::PatRecStatus::PatRecIDs );
  ...
  for( PatFwdHit* hit: track.coords() ) {
    ...
    hit->hit()->setStatus(Tf::HitBase::EStatus::UsedByPatSeeding);
    hit->setIsUsed(true);
  }
  if( UNLIKELY( msgLevel(MSG::DEBUG) ) ) {
    for( const PatFwdHit* hit: track.coords() )
                                       ...
                                       const LHCb::State* state) const
{
  ...
  for ( itH0 = rangeX0.begin(); rangeX0.end() != itH0; ++itH0 ) {
	...
	if (absdx1 < dx1best) {
      ...
      boost::container::static_vector<PatFwdHit*, 4> candsT2;
                                         ...
                                         const LHCb::State* state ) const
{
  ...
  std::sort( stereo.begin(), stereo.end(), Tf::increasingByProjection<>());
}
                                            ...
                                            std::vector<LHCb::Track*>& outputTracks ) const
{
  ...
  for(const auto* track: *inputTracks) {
    ...
    for (auto* track2: outputTracks) {
      if (track2->checkFlag(LHCb::Track::Flags::Invalid)) continue;
	...
	track2->setFlag(LHCb::Track::Flags::Invalid, true);
        break;
    ...
    LHCb::Track* out = new LHCb::Track(LHCb::Track::History::PatSeeding,
       	LHCb::Track::Types::Ttrack, LHCb::Track::PatRecStatus::PatRecIDs);
    out->setLhcbIDs( ids );
  ...
  std::transform(std::begin(outputTracks), std::end(outputTracks),
      ...
      [] (LHCb::Track* tr) {
        bool invalid = tr->checkFlag(LHCb::Track::Flags::Invalid);
